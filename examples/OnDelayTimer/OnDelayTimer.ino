...
// Create an instance of an on delay timer, with 2000ms delay. 
OnDelayTimer odt(2000);     // On delay time setpoint set to 2000ms.
// OnDelayTimer odt;        // No initial setpoint. 
...
void loop() {
    ...
    // The led will turn on 2 seconds after the button is pushed.
    // It turns off imediatly when the button is released. 
    digitalWrite(LED_BUILTIN, odt.run(buttonPushed()));

    // You can also specify the on delay time while calling run. 
